...
LexerToken parser_current(Parser* self) {
    if (self->pos >= self->count) return (LexerToken){ .tag = EOFs };
    return self->tokens[self->pos];
}

LexerToken parser_peek(Parser* self) {
    if (self->pos + 1 >= self->count) return (LexerToken){ .tag = EOFs };
...
Stmts parser_stmt(Parser* self) {
    if (self->pos >= self->count) return (Stmts){0};
...
size_t parser_find_end(Parser* self, size_t start) {
    size_t i  = start;
    int depth = 0;
    while (i < self->count) {
        LexerTokenTag tag = self->tokens[i].tag;
        if (tag == LeftParens  || tag == LeftBrackets  || tag == LeftBraces)  depth++;
        if (tag == RightParens || tag == RightBrackets || tag == RightBraces) depth--;
        if (depth == 0 && (tag == Semicolons || tag == Ends || tag == Commas || tag == EOFs)) break;
        i++;
    }
    return i;
}
...
Exprs parser_expr_range(Parser* self, size_t start, size_t end);

Exprs parser_expr_primary(Parser* self) {
    if (self->pos >= self->count) return (Exprs){0};
    ...
    if (tok.tag == Identifier) {
        ...
        if (self->pos < self->count) {
            if (parser_current(self).tag == LeftParens ||
                parser_current(self).tag == LeftBrackets)
...
Exprs parser_expr_range(Parser* self, size_t start, size_t end) {
    if (start >= end || end > self->count) return (Exprs){0};
    if (start + 1 == end) {
        self->pos = start;
        return parser_expr_primary(self);
    }
    ...
    size_t right_ptr = end;

    while (left_ptr < right_ptr) {
        right_ptr--;
        LexerTokenTag tag = self->tokens[right_ptr].tag;

        if (tag == RightParens || tag == RightBrackets || tag == RightBraces) depth++;
        if (tag == LeftParens  || tag == LeftBrackets  || tag == LeftBraces)  depth--;
        if (depth != 0) continue;

        int prec = parser_precedence(tag);
        if (prec != -1 && prec <= lowest) {
            lowest = prec;
            split  = right_ptr;
        }
    }
    ...
    if (split == (size_t)-1) {
        if (self->tokens[start].tag == LeftParens &&
            self->tokens[end-1].tag == RightParens) {
            return parser_expr_range(self, start + 1, end - 1);
        }
        self->pos = start;
        return parser_expr_primary(self);
    }

    LexerTokenTag op = self->tokens[split].tag;

    Exprs* left  = malloc(sizeof(Exprs));
    Exprs* right = malloc(sizeof(Exprs));
    if (!left || !right) { free(left); free(right); return (Exprs){0}; }
    
    *left  = parser_expr_range(self, start, split);
    *right = parser_expr_range(self, split + 1, end);

    return (Exprs){
        .tag  = Expr_BinaryOps,
        .data = { .binary_ops = { .left = left, .op = op, .right = right }}
    };
}

Exprs parser_expr(Parser* self) {
    size_t start = self->pos;
    size_t end   = parser_find_end(self, start);
    self->pos    = end;
    return parser_expr_range(self, start, end);
}
...
Exprs parser_function_call(Parser* self, SourceRange fn) {
    ...
    if (self->pos < self->count && parser_current(self).tag == LeftBrackets) {
        parser_advance(self);
        while (self->pos < self->count && parser_current(self).tag != RightBrackets && parser_current(self).tag != EOFs) {
            if (parser_current(self).tag == Identifier) {
                generic_params[generic_params_count++] = parser_current(self).range;
                parser_advance(self);
            }
            if (self->pos < self->count && parser_current(self).tag == Commas) parser_advance(self);
        }
        if (self->pos < self->count && parser_current(self).tag == RightBrackets) parser_advance(self);
    }

    if (self->pos < self->count && parser_current(self).tag == LeftParens) {
        parser_advance(self);
        while (self->pos < self->count && parser_current(self).tag != RightParens && parser_current(self).tag != EOFs) {
            Param p = {0};
            if (parser_current(self).tag == Identifier) { p.name = parser_current(self).range; parser_advance(self); }
            if (self->pos < self->count && parser_current(self).tag == Colons) parser_advance(self);
        ...
        if (self->pos < self->count && parser_current(self).tag == RightParens) parser_advance(self);
    }
...
Exprs parser_method_calls(Parser* self, SourceRange class) {
    ...
    if (self->pos < self->count && parser_current(self).tag == Identifier) {
...
Exprs parser_struct_call(Parser* self, SourceRange str) {
    ...
    if (self->pos < self->count && parser_current(self).tag == Dots) {
        parser_advance(self);
        if (self->pos < self->count && parser_current(self).tag == Identifier) { 
            function = parser_current(self).range; 
            parser_advance(self); 
    ...
    if (self->pos < self->count && parser_current(self).tag == LeftBraces) {
        parser_advance(self);
        while (self->pos < self->count && parser_current(self).tag != RightBraces && parser_current(self).tag != EOFs) {
            Param p = {0};
            if (parser_current(self).tag == Identifier) {  p.name = parser_current(self).range; parser_advance(self); }
            if (self->pos < self->count && parser_current(self).tag == Colons) parser_advance(self);
        ...
        if (self->pos < self->count && parser_current(self).tag == RightBraces) parser_advance(self);
    }
...
Type parser_type(Parser* self) {
    if (self->pos >= self->count) return (Type){ .tag = Type_Void };
    
    switch (parser_current(self).tag) {
        case Ints: {
            SourceRange r = parser_current(self).range;
            int bits = range_eq(r, "int8") ? 8 : 
                       range_eq(r, "int16") ? 16 : 
                       range_eq(r, "int64") ? 64 : 32;
            parser_advance(self);
            return (Type){ .tag = Type_Int, .data = { .int_t = { .bits = bits } } };
        }
        case Floats: {
            SourceRange r = parser_current(self).range;
            int bits = range_eq(r, "float64") ? 64 : 
                       range_eq(r, "float32") ? 32 : 32;
            parser_advance(self);
...
Stmts parser_functions(Parser* self, bool is_const, bool is_unsafe, bool is_pub) {
    ...
    if (self->pos < self->count && parser_current(self).tag == LeftParens) { parser_advance(self); }

    while (self->pos < self->count && parser_current(self).tag != RightParens) {
        Param p = {0};
        if (parser_current(self).tag == Identifier) { 
            p.name = parser_current(self).range; 
            parser_advance(self); 
    ...
    if (self->pos < self->count && parser_current(self).tag == RightParens) parser_advance(self);
    if (self->pos < self->count && parser_current(self).tag == Colons) { 
        parser_advance(self); 
        return_type = parser_type(self); 
    }

    while (self->pos < self->count && parser_current(self).tag != Ends && parser_current(self).tag != EOFs) { 
        body[body_count++] = parser_stmt(self); 
    }
    if (self->pos < self->count && parser_current(self).tag == Ends) parser_advance(self);
...
Stmts parser_class(Parser* self, bool is_pub) {
    ...
    if (self->pos < self->count && parser_current(self).tag == Fors) {
        parser_advance(self);
        while (self->pos < self->count && parser_current(self).tag == Identifier) {
    ...
    if (self->pos < self->count && parser_current(self).tag == Greaters) {
        parser_advance(self);
        if (self->pos < self->count && parser_current(self).tag == Identifier) { 
    ...
    while (self->pos < self->count && parser_current(self).tag != Ends && parser_current(self).tag != EOFs) {
        if (parser_current(self).tag == Vars || parser_current(self).tag == Lets) {
            ...
            if (self->pos < self->count && parser_current(self).tag == Identifier) { f.name = parser_current(self).range;  parser_advance(self); }
            if (self->pos < self->count && parser_current(self).tag == Colons) { parser_advance(self); }
    ...
    while (self->pos < self->count && parser_current(self).tag != Ends && parser_current(self).tag != EOFs) {
        ...
        switch (parser_current(self).tag) {
            case Vars: {
                ...
                if (self->pos < self->count && parser_current(self).tag == Identifier) { f.name = parser_current(self).range;  parser_advance(self); }
                if (self->pos < self->count && parser_current(self).tag == Equalss) { parser_advance(self); }
