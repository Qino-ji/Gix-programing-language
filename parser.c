...
Stmts parser_class(Parser* self, bool is_pub) {
    ...
    while (parser_current(self).tag != Ends && parser_current(self).tag != EOFs) {
        if (parser_current(self).tag == Vars || parser_current(self).tag == Lets) {
        ...
        } else if (parser_current(self).tag == Functions) {} else if (parser_current(self).tag == Ats) {
            parser_advance(self);

            // Now we should be on "operation" identifier
            ...
            // NOW parse the op — this consumes "operation(+=)"
            Operation op = operation_op(self);

            // Now we should be on "func"
